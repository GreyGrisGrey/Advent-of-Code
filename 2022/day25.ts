...
function snafu(num: string): number {
    let newNum = 0
...
function part1(): string {
    let num = fs.readFileSync("in.txt", "utf8").split("\r\n").map((x) => snafu(x)).reduce((base, curr) => base + curr)
    let snafuNum = "", digit = 1
    while (num > 0) {
        let stepVal = (num % (Math.pow(5, digit))) / (Math.pow(5, Math.max(digit - 1, 0)))
        if (stepVal == 3) {
